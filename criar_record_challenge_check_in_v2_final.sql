...
-- Usa a estrutura correta da tabela check_in_error_logs
-- Corrigido para usar check_ins_count em challenge_progress e total_check_ins em user_progress
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      SELECT 
        consecutive_days,
        cp.last_check_in,
        COALESCE(cp.check_ins_count, 0) AS check_ins_count
      INTO current_progress
      FROM challenge_progress cp
      WHERE 
        cp.challenge_id = challenge_id_param AND 
      ...
      ELSE
        ...
        current_check_ins := current_progress.check_ins_count + 1;
      END IF;
    END;
    ...
    BEGIN
      ...
      IF progress_exists THEN
        ...
        SET 
          points = cp.points + points_awarded,
          check_ins_count = current_check_ins,
          consecutive_days = streak_count,
          last_check_in = date_param,
          updated_at = NOW(),
          user_name = profiles.name,
          user_photo_url = profiles.photo_url
        FROM profiles 
        WHERE 
          cp.challenge_id = challenge_id_param AND 
          cp.user_id = user_id_param AND
          profiles.id = user_id_param;
      ELSE
    ...
    BEGIN
      ...
      IF user_progress_exists THEN
        ...
        SET 
          ...
          updated_at = NOW(),
          level = CASE 
            WHEN (points + points_awarded) / 100 > level THEN (points + points_awarded) / 100
            ELSE level
          END
        WHERE user_id = user_id_param;
      ELSE
    ...
    WITH ranked_participants AS (
      SELECT
        ...
        ROW_NUMBER() OVER (
          ...
          ORDER BY points DESC, cp.last_check_in ASC
        ) AS new_position
      FROM challenge_progress cp
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Registrar o erro em log - Usando a estrutura correta da tabela
...
-- Remover código de compatibilidade que pode estar causando confusão
-- Cada tabela deve usar seu próprio nome de coluna sem aliases 