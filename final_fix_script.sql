-- Script SQL para corrigir problemas do Ray Club App
-- Colunas faltantes identificadas pelo diagnóstico

-- 1. Adicionar colunas faltantes na tabela user_progress
...
-- 2. Resolver nomenclatura entre points e total_points
COMMENT ON COLUMN user_progress.points IS 'Pontos totais do usuário (usado como total_points no código)';

-- 3. Criar a nova versão da função get_dashboard_data que é resiliente a diferenças de estrutura
...
BEGIN
    -- Get user progress with safe column access
    SELECT json_build_object(
        ...
        'total_points', COALESCE(up.points, 0), -- Usa points, mas retorna como total_points
        'days_trained_this_month', COALESCE(up.days_trained_this_month, 0),
        'workout_types', COALESCE(up.workouts_by_type, '{}'::jsonb),
    ...
    -- Get challenge progress for the current challenge (handling column name inconsistencies)
    SELECT 
        CASE WHEN current_challenge_data IS NOT NULL THEN
            (SELECT json_build_object(
                ...
                'total_check_ins', COALESCE(GREATEST(cp.check_ins_count, cp.total_check_ins), 0),
