-- Ray Club — Correção Final do Ranking de Cardio
-- Remove funções duplicadas e recria a versão correta

-- 1. Remover todas as versões da função
DROP FUNCTION IF EXISTS public.get_cardio_ranking(timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.get_cardio_ranking(timestamptz, timestamptz, integer, integer);

-- 2. Criar função corrigida e única
...
-- 4. Verificar se há participantes
SELECT 'Participantes ativos:' as info, count(*) as total
FROM public.cardio_challenge_participants 
WHERE active = true;

-- 5. Verificar treinos de cardio
SELECT 'Treinos de Cardio:' as info, count(*) as total
