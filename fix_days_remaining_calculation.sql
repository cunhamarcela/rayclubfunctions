-- Script para corrigir o cálculo de dias restantes dos desafios
-- Problema: O cálculo atual não considera o dia atual como um dia restante
-- Solução: Adicionar +1 ao cálculo de EXTRACT(DAY FROM (end_date - CURRENT_DATE))

-- 1. Backup da função atual
CREATE OR REPLACE FUNCTION get_dashboard_data_backup(user_id_param UUID)
RETURNS JSON AS $$
BEGIN
    -- Esta é uma função de backup - usar get_dashboard_data original se necessário
    RETURN '{"error": "Esta é uma função de backup, use get_dashboard_data"}'::json;
END;
$$ LANGUAGE plpgsql;

-- 2. Atualizar a função principal com o cálculo correto
    ...
    SELECT 
        CASE WHEN COUNT(*) > 0 THEN
            (SELECT json_build_object(
                ...
                -- CORREÇÃO: Adicionar +1 ao cálculo de dias restantes
...
COMMENT ON FUNCTION get_dashboard_data(UUID) IS 'Função principal para buscar todos os dados do dashboard enhanced. CORREÇÃO APLICADA: days_remaining agora inclui o dia atual (+1) para contagem correta dos dias restantes do desafio.';

-- 4. Verificação da correção
DO $$
BEGIN
    RAISE NOTICE 'Correção aplicada com sucesso!';
    RAISE NOTICE 'O cálculo de days_remaining agora usa: EXTRACT(DAY FROM (end_date - CURRENT_DATE)) + 1';
    RAISE NOTICE 'Isso significa que o dia atual conta como um dia restante.';
    RAISE NOTICE 'Exemplo: Se hoje é 31/05 e desafio termina em 15/06, mostrará 16 dias (correto).';
END $$; 