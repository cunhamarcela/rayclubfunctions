...
import '../screens/challenge_ranking_screen.dart';
...
// Adicionando o provider para o usuário atual usando o Supabase diretamente
...
final userGroupsProvider = FutureProvider.family<List<ChallengeGroup>, String?>((ref, userId) async {
  ...
  // Se já temos grupos carregados, retorne-os imediatamente
  if (groupState.groups.isNotEmpty) {
    return groupState.groups;
  }
  
  // Se ainda não temos grupos, carregue-os em um postFrameCallback para evitar modificar 
  // o estado durante a construção
...
class _ChallengeDetailScreenState extends ConsumerState<ChallengeDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      debugPrint('🔍 ChallengeDetailScreen - postFrameCallback para id: ${widget.challengeId}');
  ...
  Widget build(BuildContext context) {
    ...
    // Definir userProgress e isActive aqui para que estejam disponíveis no bottomSheet
    final now = DateTime.now();
    final isActive = challenge?.endDate.isAfter(now) ?? false;
    final userProgress = challengeState.userProgress;

    // Diagnóstico
    debugPrint('🔍 ChallengeDetailScreen - build chamado');
    debugPrint('🔍 ChallengeDetailScreen - challengeId: ${widget.challengeId}');
    debugPrint('🔍 ChallengeDetailScreen - challenge carregado: ${challenge?.title}');
    debugPrint('🔍 ChallengeDetailScreen - isLoading: ${challengeState.isLoading}');
    debugPrint('🔍 ChallengeDetailScreen - progresso: ${challengeState.progressList.length} participantes');
    debugPrint('🔍 ChallengeDetailScreen - filtro: ${challengeState.selectedGroupIdForFilter}');
    debugPrint('🔍 ChallengeDetailScreen - userId: $userId');

    // Handle loading and error states for the main challenge data
    if (challengeState.isLoading && challenge == null) {
      debugPrint('🔍 ChallengeDetailScreen - exibindo loading...');
      return const Scaffold(body: LoadingIndicator());
    }

    if (challenge == null) {
      debugPrint('❌ ChallengeDetailScreen - challenge não encontrado ou com erro');
      return Scaffold(
        appBar: AppBar(
          title: Text(
            'TESTE MODIFICAÇÃO - Desafio',
        ...
        body: EmptyState(
          message: challengeState.errorMessage ?? 'Desafio não encontrado ou erro ao carregar.',
  ...
  Widget _buildContent(
    ...
    // Garantimos que selectedChallenge não será nulo, pois este método
    // só é chamado após verificarmos isso na build
    final challenge = challengeState.selectedChallenge;
    if (challenge == null) {
      // Como precaução adicional, mostramos um widget vazio caso challenge seja nulo
          ...
          flexibleSpace: FlexibleSpaceBar(
            background: Stack(
              ...
              children: [
                ...
                // Título agora está posicionado mais baixo, sem a tag Oficial
                Positioned(
                  bottom: 25,
                  left: 16,
                  right: 16,
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Error state with retry button
                          ...
                          Padding(
                            ...
                            child: RetryRankingButton(
                              ...
                              onAfterRetry: () {
                                // Opcional: mostrar snackbar ou fazer algo após tentar novamente
                        ...
                        // Botão para login se usuário não estiver autenticado
                        if (userId == null) ...[
                          const SizedBox(height: 32),
                          SizedBox(
                            width: double.infinity,
  ...
  Widget _buildStatusBadge(bool isActive, int daysLeft) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
      decoration: BoxDecoration(
        color: isActive ? AppColors.pastelYellow : AppColors.lightGray,
        borderRadius: BorderRadius.circular(20),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          // Botões de filtro para cada grupo
  ...
  // Método completamente reescrito para navegação direta
  void _navigateToFullRanking(BuildContext context, String? challengeId) {
    if (challengeId == null || challengeId.isEmpty) {
      print('❌ DIAGNÓSTICO: Tentativa de navegar para ranking com challengeId nulo ou vazio');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Erro: Identificador do desafio não encontrado'),
    ...
    print('🔍 DIAGNÓSTICO: Tentando navegar para ranking completo do desafio $challengeId');
    
    try {
      // Navegar usando AutoRoute (sistema de navegação correto do app)
      print('🔍 DIAGNÓSTICO: Tentando método de navegação com AutoRoute');
      context.router.pushNamed('/challenges/ranking/$challengeId');
    } catch (e, stackTrace) {
      print('❌ DIAGNÓSTICO: Exceção ao tentar navegar para ranking completo: $e');
      print('❌ DIAGNÓSTICO: Stack trace: $stackTrace');
      
      // Fallback para o método anterior se o Auto Route falhar
      try {
        print('🔍 DIAGNÓSTICO: Tentando método de navegação de fallback - MaterialPageRoute direto');
        ...
        print('❌ DIAGNÓSTICO: Erro também no método de fallback: $e2');
      }
    }
  }

  // Método para confirmar saída do desafio
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('Tem certeza que deseja deixar este desafio? Seu progresso será perdido.'),
          actions: [
            TextButton(
            ...
            TextButton(
              onPressed: () {
                ...
                // Implementar a lógica para deixar o desafio aqui
  ...
  void _joinChallenge(BuildContext context, String challengeId) {
    ...
    if (userId != null) {
      ...
      // PATCH: Corrigir bug 6 - Implementar a função de participar do desafio
      ref.read(challengeViewModelProvider.notifier).joinChallenge(
        ...
        // Força recarregamento de dados completo
        Future.delayed(const Duration(milliseconds: 300), () {
          // Navegar novamente para a tela de detalhes para garantir atualização completa
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Você precisa estar logado para participar do desafio'),
          backgroundColor: AppColors.orangeDark,
        ),
      );
    }
  ...
  // Método para navegação para a tela de treinos do desafio
  void _navigateToChallengeWorkouts(BuildContext context, String? challengeId) {
    if (challengeId == null || challengeId.isEmpty) {
      print('❌ DIAGNÓSTICO: Tentativa de navegar para treinos com challengeId nulo ou vazio');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Erro: Identificador do desafio não encontrado'),
          backgroundColor: Colors.red,
    ...
    print('🔍 DIAGNÓSTICO: Tentando navegar para workouts do desafio $challengeId');
    
    try {
      // Primeiro método: navegação via AutoRoute com path nomeado
      context.pushRoute(ChallengeWorkoutsRoute(challengeId: challengeId));
    } catch (e) {
      print('⚠️ DIAGNÓSTICO: Falha no primeiro método de navegação: $e');
      
      try {
        // Segundo método: navegação via path string
        context.router.pushNamed('/challenges/workouts/$challengeId');
      } catch (e2, stackTrace) {
        print('❌ DIAGNÓSTICO: Exceção ao tentar navegar para workouts: $e2');
        print('❌ DIAGNÓSTICO: Stack trace: $stackTrace');
        
        // Método de fallback: navegação direta
        try {
          Navigator.of(context).push(
            MaterialPageRoute(
          ...
          print('❌ DIAGNÓSTICO: Todos os métodos de navegação falharam: $e3');
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
