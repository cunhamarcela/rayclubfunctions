-- Script para corrigir o cálculo da semana atual na função get_dashboard_fitness
-- O problema é que está usando os últimos 7 dias em vez da semana atual (segunda a domingo)

-- Primeiro, vamos fazer backup da função atual
-- CREATE OR REPLACE FUNCTION get_dashboard_fitness_backup AS (cópia da função atual)

-- Agora vamos corrigir apenas a parte do cálculo da semana
...
BEGIN
    ...
    -- Definir início da semana atual (segunda-feira)
    ...
    -- ESTATÍSTICAS: Dados da semana atual (CORRIGIDO)
    week_stats AS (
        SELECT 
...
SELECT 'Função get_dashboard_fitness corrigida com sucesso!' as status; 