-- =====================================================
-- SCRIPT: CORRIGIR PDF DO TREINO G
-- =====================================================
-- Data: 2025-01-21
-- Objetivo: Verificar e adicionar PDF ao Treino G se estiver faltando
-- =====================================================
...
FROM materials m
LEFT JOIN workout_videos wv ON m.video_id = wv.id
WHERE m.type = 'pdf' 
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (
      SELECT 1 FROM materials m
      JOIN workout_videos wv ON m.video_id = wv.id
      WHERE LOWER(wv.title) LIKE '%treino g%'
        AND m.type = 'pdf'
        AND (wv.category = '495f6111-00f1-4484-974f-5213a5a44ed8' OR wv.category = 'Musculação')
    ) 
...
SELECT 
  ...
  CASE 
    ...
    ELSE '❌ SEM PDF' 
  END as status_pdf,
  (SELECT m.file_path FROM materials m WHERE m.video_id = wv.id AND m.type = 'pdf' LIMIT 1) as pdf_path
FROM workout_videos wv 
WHERE LOWER(wv.title) LIKE '%treino g%'
  AND (wv.category = '495f6111-00f1-4484-974f-5213a5a44ed8' OR wv.category = 'Musculação')
  AND NOT LOWER(wv.title) LIKE '%semana%';
...
SELECT 
  ...
  CASE 
    WHEN COUNT(*) FILTER (WHERE (SELECT COUNT(*) FROM materials WHERE video_id = wv.id AND type = 'pdf') = 0) = 0 
