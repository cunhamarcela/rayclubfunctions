-- =====================================================
-- SCRIPT: CORRIGIR PDF DO TREINO G (VERSÃO CORRIGIDA)
-- =====================================================
-- Data: 2025-01-21
-- Objetivo: Verificar e adicionar PDF ao Treino G - versão corrigida com tipos
-- =====================================================
...
SELECT 
  ...
  CASE 
    ...
    ELSE '❌ SEM PDF' 
  END as status_pdf,
  (SELECT m.file_path FROM materials m WHERE m.video_id::text = wv.id::text AND m.type = 'pdf' LIMIT 1) as pdf_path
FROM workout_videos wv 
WHERE LOWER(wv.title) LIKE '%treino g%'
  AND (wv.category = '495f6111-00f1-4484-974f-5213a5a44ed8' OR wv.category = 'Musculação')
  AND NOT LOWER(wv.title) LIKE '%semana%';
...
SELECT 
  ...
  CASE 
    WHEN COUNT(*) FILTER (WHERE (SELECT COUNT(*) FROM materials WHERE video_id::text = wv.id::text AND type = 'pdf') = 0) = 0 
