-- ================================================================
-- FUNÇÕES SQL PARA INTEGRAÇÃO COM STRIPE WEBHOOK
-- Data: 2025-01-19
-- Objetivo: Automatizar atualização de usuários para expert após pagamento
-- ================================================================
...
-- 2. FUNÇÃO PARA ATUALIZAR USUÁRIO POR EMAIL
...
BEGIN
  ...
  INSERT INTO payment_logs (
    ...
    'user_level_update', new_level, expires_at, 'pending'
  ) RETURNING id INTO log_id;

  -- Buscar usuário pelo email
  SELECT auth.uid INTO user_id_found
  FROM auth.users 
  WHERE email = email_param 
  LIMIT 1;

  -- Se usuário não encontrado, tentar na tabela profiles
  ...
  IF user_id_found IS NULL THEN
    ...
    INSERT INTO pending_user_levels (
      ...
      email_param, new_level, expires_at, stripe_customer_id, stripe_subscription_id
    ) ON CONFLICT (email) 
    DO UPDATE SET 
      level = new_level,
...
-- 3. FUNÇÃO PARA PROCESSAR USUÁRIOS PENDENTES
...
-- 4. FUNÇÃO PARA VERIFICAR STATUS DE PAGAMENTO
...
BEGIN
  ...
  JOIN auth.users u ON u.id = upl.user_id
  ...
  result := json_build_object(
    ...
    'expires_at', user_level_data.level_expires_at,
    ...
    'has_access', (user_level_data.level_expires_at IS NULL OR user_level_data.level_expires_at > NOW())
  );
...
-- 5. TRIGGER PARA PROCESSAR USUÁRIOS PENDENTES AUTOMATICAMENTE
...
DROP TRIGGER IF EXISTS trigger_process_pending_on_signup ON auth.users;
