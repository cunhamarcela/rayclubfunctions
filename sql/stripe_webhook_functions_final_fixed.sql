-- ================================================================
-- FUNÇÕES SQL PARA INTEGRAÇÃO COM STRIPE WEBHOOK - VERSÃO FINAL CORRIGIDA
-- Data: 2025-01-19
-- Fix: DROP funções existentes primeiro para resolver conflito de parâmetros
-- ================================================================

-- IMPORTANTE: Fazer DROP das funções existentes primeiro
DROP FUNCTION IF EXISTS update_user_level_by_email(text,text,timestamp,text,text,text);
DROP FUNCTION IF EXISTS update_user_level_by_email(text,text,timestamp without time zone,text,text,text);
DROP FUNCTION IF EXISTS check_payment_status(text);
DROP FUNCTION IF EXISTS process_pending_user_levels();
DROP FUNCTION IF EXISTS trigger_process_pending_users();
DROP FUNCTION IF EXISTS cleanup_old_payment_logs();

-- 1. TABELA DE LOGS DE PAGAMENTOS
...
-- 2. FUNÇÃO PRINCIPAL - TOTALMENTE NOVA
...
BEGIN
  ...
  -- Buscar usuário que existe em AMBAS as tabelas (profiles E auth.users)
  ...
  -- Verificar se user_id existe em auth.users antes de inserir
...
-- 3. FUNÇÃO PARA VERIFICAR STATUS DE PAGAMENTO
CREATE FUNCTION check_payment_status(email_param TEXT)
RETURNS JSON AS $$
DECLARE
  ...
  result JSON;
BEGIN
  -- Buscar dados do usuário usando aliases específicos
  SELECT 
    upl.current_level,
...
-- 4. FUNÇÃO PARA PROCESSAR USUÁRIOS PENDENTES
...
-- 5. TRIGGER PARA PROCESSAR USUÁRIOS PENDENTES
...
-- 6. FUNÇÃO PARA CLEANUP DE LOGS ANTIGOS
...
-- CORREÇÕES APLICADAS:
-- ✅ DROP das funções existentes para evitar conflito de parâmetros
-- ✅ Usa INNER JOIN para garantir ID existe em auth.users
-- ✅ Resolve ambiguidade com aliases específicos
-- ✅ Validação extra antes de inserir em user_progress_level
-- ✅ Fallback robusto com sistema de pendentes

-- TESTE RÁPIDO:
SELECT 'Funções criadas com sucesso!' as status;

-- Para usar imediatamente:
-- SELECT update_user_level_by_email('email@usuario.com', 'expert', (NOW() + INTERVAL '30 days')::timestamp); 