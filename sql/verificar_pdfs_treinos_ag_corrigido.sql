-- =====================================================
-- SCRIPT: VERIFICAR PDFs DOS TREINOS A-G (VERSÃO CORRIGIDA)
-- =====================================================
-- Data: 2025-01-21
-- Objetivo: Verificar se todos os treinos A-G têm PDFs corretos - versão corrigida com tipos
-- =====================================================
...
SELECT 
  ...
  CASE 
    ...
    ELSE 'OUTRO'
  END as treino_letra,
  (SELECT COUNT(*) FROM materials WHERE video_id::text = wv.id::text AND type = 'pdf') as pdf_count,
  CASE 
    WHEN (SELECT COUNT(*) FROM materials WHERE video_id::text = wv.id::text AND type = 'pdf') > 0 THEN '✅ TEM PDF' 
    ELSE '❌ SEM PDF' 
  END as status_pdf,
  (SELECT STRING_AGG(m.title, ', ') FROM materials m WHERE m.video_id::text = wv.id::text AND m.type = 'pdf') as pdf_titles
...
WHERE (wv.category = '495f6111-00f1-4484-974f-5213a5a44ed8' OR wv.category = 'Musculação')
  ...
  AND NOT LOWER(wv.title) LIKE '%semana%'
  AND (SELECT COUNT(*) FROM materials WHERE video_id::text = wv.id::text AND type = 'pdf') = 0;
...
WHERE m.type = 'pdf'
  ...
  AND NOT EXISTS (
    ...
    WHERE wv.id::text = m.video_id::text
  );
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE (SELECT COUNT(*) FROM materials WHERE video_id::text = wv.id::text AND type = 'pdf') > 0) as com_pdf,
  COUNT(*) FILTER (WHERE (SELECT COUNT(*) FROM materials WHERE video_id::text = wv.id::text AND type = 'pdf') = 0) as sem_pdf
FROM workout_videos wv 
WHERE (wv.category = '495f6111-00f1-4484-974f-5213a5a44ed8' OR wv.category = 'Musculação')
  AND (
    LOWER(wv.title) LIKE '%treino a%' OR
